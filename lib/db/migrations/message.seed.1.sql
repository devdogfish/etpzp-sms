...
WITH insert_message AS (
    INSERT INTO message (user_id, subject, body, status, location, failure_reason) 
    VALUES (8, 'Meme Coins & Fries', 'Meme coins are a lot like dessert – sweet, fun, but a bit unpredictable. You never really know if you’ll get a sweet payoff or end up with a bad taste. Right now, I’m hoping for a Dogecoin surge while I indulge in some fries. If they both hit, I’ll be in heaven.', 'sent', 'sent', NULL) 
    RETURNING id
)
INSERT INTO recipient (message_id, phone)
SELECT 
...
WITH insert_message AS (
    INSERT INTO message (user_id, subject, body, status, location, failure_reason) 
    VALUES (8, 'Shiba Inu & Burger Thoughts', 'You ever feel like you’re just chasing that next big break with meme coins? Right now, my Shiba Inu is doing well, but I’m not sure if I should sell for a quick profit or hold for the moon. In the meantime, I’m enjoying my burger, trying not to think about it too much.', 'sent', 'sent', NULL) 
    RETURNING id
)
INSERT INTO recipient (message_id, phone)
SELECT 
...
WITH insert_message AS (
    INSERT INTO message (user_id, subject, body, status, location, failure_reason) 
    VALUES (8, 'Shiba Inu Sushi Roll', 'I was enjoying some sushi when I thought, Is Shiba Inu as volatile as this spicy tuna roll? Sometimes it’s a risk, but when it’s good, it’s amazing. I’m still holding onto my Shiba. I’d like to think it’s the sushi of meme coins — unique, tasty, and worth the gamble if you get it at the right time!', 'sent', 'sent', NULL) 
    RETURNING id
)
INSERT INTO recipient (message_id, phone)
SELECT 
...
WITH insert_message AS (
    INSERT INTO message (user_id, subject, body, status, location, failure_reason) 
    VALUES (8, 'Shiba Inu Pizza Delivery', 'I’ve been holding Shiba Inu for what feels like an eternity. It’s like waiting for a good pizza delivery: sometimes it’s delayed, sometimes it’s cold, but when it hits, it’s worth it. I just hope Shiba Inu starts delivering like my favorite pizza joint. Ready for that big price surge to finally come in!', 'sent', 'sent', NULL) 
    RETURNING id
)
INSERT INTO recipient (message_id, phone)
SELECT 
...
WITH insert_message AS (
    INSERT INTO message (user_id, subject, body, status, location, failure_reason) 
    VALUES (8, 'SafeMoon Nachos!', 'Just traded some SafeMoon while munching on some nachos. Trading meme coins is like diving into a big plate of nachos — there’s a lot going on, and you never really know what’s coming next. But if you’re careful, you get a lot of flavor in the end. Here’s to hoping SafeMoon tastes like victory soon!', 'sent', 'sent', NULL) 
    RETURNING id
)
INSERT INTO recipient (message_id, phone)
SELECT 
